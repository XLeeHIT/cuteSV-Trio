...
def parseArgs(argv):
	...
	parser.add_argument('--run_TRA',
		...
		action="store_true")
	parser.add_argument('--output_haplotype_reads',
		help = "Output SV related haplotype construction read names.",
		action="store_true")
	...
	# **************Parameters in phasing******************
	GroupPhasing = parser.add_argument_group('Phasing')
	GroupPhasing.add_argument('--performing_phasing',
		help = "The option of performing structural variant phasing.",
		action="store_true")
	GroupPhasing.add_argument('--parents_phasing',
		help = "The option of performing parents' structural variant phasing.",
		action="store_true")
	GroupPhasing.add_argument('--multiple_phasing',
		help = "The option of performing phasing again after haplotype genotype correction.",
		action="store_true")
	GroupPhasing.add_argument('--phase_all_ctgs',
		help = "Call variants on all contigs, otherwise call in chr{1..22,X,Y} and {1..22,X,Y}.",
		action="store_true")
	GroupPhasing.add_argument('--read_pos_interval',
		help = "Position division interval of the read position. [%(default)s]",
		default = 1000,
        type = int)
	

	# **************Parameters in local assembly******************
	GroupAssembly = parser.add_argument_group('Assembly')
	GroupAssembly.add_argument('--performing_assembly',
		help = "The option of performing local assembly correction.",
		action="store_true")
	GroupAssembly.add_argument('--family_assembly_reads',
		help = "The option of generating supplementary assembly reads from other family members. The supplementary fq files are saved in the assembly.fq.results folder.",
		action="store_true")
	GroupAssembly.add_argument('--remap_merge_k',
		help = "Position division interval of the read position. [%(default)s]",
		default = 25,
        type = int)
	GroupAssembly.add_argument('--remap_minimizer_window',
		help = "Position division interval of the read position. [%(default)s]",
		default = 15,
        type = int)
	GroupAssembly.add_argument('--assembly_correction_threshold',
		help = "Position division interval of the read position. [%(default)s]",
		default = 80,
        type = int)
	GroupAssembly.add_argument('--assembly_correction_setting',
		help = "The setting of assembly correction functions. [%(default)s]",
		default = "ALL",
        type = str)
	GroupAssembly.add_argument('--assembly_accelerate',
		help = "The option of accelerating local assembly correction by skipping the consensus of excessively long reads .",
		action="store_true")
	GroupAssembly.add_argument('--similarity_supplement_threshold',
		help = "The similarity threshold for generating parental hereditary shared supplementary sequences. [%(default)s]",
		default = 0.0025,
        type = float)
