...
def genetic_phasing_family(path, chr, candidate_single_SV_gt_fam_ls, family_mode, read_pos_interval, minimum_support_reads_list, phase_all_ctgs, parents_phasing, family_bams, remap_merge_k, remap_minimizer_window, assembly_correction_threshold, performing_assembly, multiple_phasing) :
    ...
    if phase_all_ctgs == True or (phase_all_ctgs == False and chr in chr_ls) :
        ...
        while(phasing_again_flag) :
            ...
            if change_num < 10 or multiple_phasing == False:
    ...
    if family_mode == "M1" :
        ...
        for j in range(len(candidate_single_SV_gt_fam_ls[0])) :
            if phase_flag :
                ...
                single_SV_gt[gt_index] = single_SV_gt[gt_index].split(":")[0] + ":" + str(round(phased_sv_haplotype_child_father[j][0])) + "|" + str(round(phased_sv_haplotype_child_mother[j][0]))
                ...
                if parents_phasing :
                    ...
                    single_SV_gt[gt_index] = single_SV_gt[gt_index].split(":")[0] + ":" + str(round(phased_sv_haplotype_father_inher[j][0])) + "|" + str(round(phased_sv_haplotype_father_forgo[j][0]))
                    ...
                    single_SV_gt[gt_index] = single_SV_gt[gt_index].split(":")[0] + ":" + str(round(phased_sv_haplotype_mother_inher[j][0])) + "|" + str(round(phased_sv_haplotype_mother_forgo[j][0]))
...
def phasing_sv_error_detection_correction(chr, candidate_single_SV_gt_fam_ls, phased_sv_haplotype_child_father, phased_sv_haplotype_child_mother, phased_sv_haplotype_father_inher, phased_sv_haplotype_father_forgo, phased_sv_haplotype_mother_inher, phased_sv_haplotype_mother_forgo, family_mode, minimum_support_reads_list) :
    ...
    if family_mode == "M1" :
        ...
        haplotype_mother_ls = [phased_sv_haplotype_child_mother,phased_sv_haplotype_father_forgo,phased_sv_haplotype_mother_forgo]
    elif family_mode == "M2" :
        haplotype_father_ls = [phased_sv_haplotype_child_father,phased_sv_haplotype_father_inher]
        haplotype_mother_ls = [phased_sv_haplotype_child_mother,phased_sv_haplotype_father_forgo]
    ...
    for i in range(len(candidate_single_SV_gt_fam_ls)) :
        ...
        for j in range(len(candidate_single_SV_gt_fam_ls[i])) :
            if (len(haplotype_father[j][10]) != 0 or len(haplotype_father[j][11]) != 0) and max([int(x) for x in candidate_single_SV_gt_fam_ls[i][j][gl_index].split(",")[0:3]]) != 100 :
                ...
                _, GL_P_F_1, _, _, _, _ = cal_GL_2(len(haplotype_father[j][11]), len(haplotype_father[j][10]))
                _, GL_P_M_1, _, _, _, _ = cal_GL_2(len(haplotype_mother[j][11]), len(haplotype_mother[j][10]))
    ...
    #        logging.info("%s/%s"%(str(candidate_single_SV_gt_fam_ls[0][i][2]),str(mother_len_ls)))
        if candidate_single_SV_gt_fam_ls[0][i][gt_index][0:3] == "1/1" :
            ...
            if DV == 0 :
                continue
            if DR == 0 :
                if len(phased_sv_haplotype_child_mother[i][6]) == 0 and len(phased_sv_haplotype_child_father[i][6]) / DV > correction_rate_threshold:
                    ...
                    phased_sv_haplotype_child_father[i][6] = candidate_single_SV_gt_fam_ls[0][i][12].split(",")
                    phased_sv_haplotype_child_father[i][7] = candidate_single_SV_gt_fam_ls[0][i][15].split(",")
                    ...
                    change_num += 1
                if len(phased_sv_haplotype_child_father[i][6]) == 0 and len(phased_sv_haplotype_child_mother[i][6]) / DV > correction_rate_threshold:
                    ...
                    gl_split_ls[-1] = "-4"
                    candidate_single_SV_gt_fam_ls[0][i][gl_index] = ",".join(gl_split_ls)
                    phased_sv_haplotype_child_mother[i][6] = candidate_single_SV_gt_fam_ls[0][i][12].split(",")
                    phased_sv_haplotype_child_mother[i][7] = candidate_single_SV_gt_fam_ls[0][i][15].split(",")
                    phased_sv_haplotype_child_father[i][8] = candidate_single_SV_gt_fam_ls[0][i][16].split(",")
...
def eliminate_phase_information(candidate_single_SV_gt_fam_ls) :
    gt_index = 8
    for i in range(len(candidate_single_SV_gt_fam_ls[0])) :
        candidate_single_SV_gt_fam_ls[0][i][gt_index] = candidate_single_SV_gt_fam_ls[0][i][gt_index][0:3]

def generate_haplotype_read_names(temporary_dir, family_mode, phasing_fam_results, child_hap1_results, child_hap2_results, father_hap1_results, father_hap2_results, mother_hap1_results, mother_hap2_results) :
    gt_index = 8
    child_hap1_write_file = temporary_dir + "/child_hap1_reads.txt"
    with open(child_hap1_write_file,'w') as f:
        for chr in child_hap1_results.keys():
            phased_sv_haplotype_child_father = child_hap1_results[chr]
            candidate_single_SV_gt_ls = phasing_fam_results[0][chr]
            for i in range(len(phased_sv_haplotype_child_father)) :
    ...
    child_hap2_write_file = temporary_dir + "/child_hap2_reads.txt"
    with open(child_hap2_write_file,'w') as f:
        for chr in child_hap2_results.keys():
            phased_sv_haplotype_child_mother = child_hap2_results[chr]
            candidate_single_SV_gt_ls = phasing_fam_results[0][chr]
    ...
    father_hap1_write_file = temporary_dir + "/first_parent_hap1_reads.txt"
    with open(father_hap1_write_file,'w') as f:
        for chr in father_hap1_results.keys():
            phased_sv_haplotype_father_inher = father_hap1_results[chr]
            candidate_single_SV_gt_ls = phasing_fam_results[0][chr]
    ...
    father_hap2_write_file = temporary_dir + "/first_parent_hap2_reads.txt"
    with open(father_hap2_write_file,'w') as f:
        for chr in father_hap2_results.keys():
            phased_sv_haplotype_father_forgo = father_hap2_results[chr]
            candidate_single_SV_gt_ls = phasing_fam_results[0][chr]
    ...
    if family_mode == "M1" :
        mother_hap1_write_file = temporary_dir + "/second_parent_hap1_reads.txt"
        with open(mother_hap1_write_file,'w') as f:
            for chr in mother_hap1_results.keys():
                phased_sv_haplotype_mother_inher = mother_hap1_results[chr]
                candidate_single_SV_gt_ls = phasing_fam_results[0][chr]
        ...
        mother_hap2_write_file = temporary_dir + "/second_parent_hap2_reads.txt"
        with open(mother_hap2_write_file,'w') as f:
            for chr in mother_hap2_results.keys():
                phased_sv_haplotype_mother_forgo = mother_hap2_results[chr]
                candidate_single_SV_gt_ls = phasing_fam_results[0][chr]
