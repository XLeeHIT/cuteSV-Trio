...
# 按照SV是否为某种denovo变异进行gt修正
def correction_mendel(candidate_single_SV_gt_fam_ls, family_mode, nearby_matching, minimum_support_reads_list) :
    #pass
    gl_index = 9
    gt_index = 8
    qual_index = 11
    for i in range(len(candidate_single_SV_gt_fam_ls[0])) :
        ...
        if resolution_denovo(family_gt_ls, family_gl_ls[0], family_mode) :
            ...
            if family_genotype_ls[0] in ["0/1","1/0"] and "0/0" == family_genotype_ls[1] and "0/0" == family_genotype_ls[2] :
                if gl_split_ls[8] in ["-1","-2","-3"] :
                    continue
                correct_flag = -1
                #if float(candidate_single_SV_gt_fam_ls[0][i][qual_index]) <= 10 :
                #    correct_flag = True
                #if int(float(gl_split_ls[7])) <= 1 * minimum_support_reads_list[0] and int(float(fa_split_ls[7])) <= 0.5 * minimum_support_reads_list[1] and int(float(mo_split_ls[7])) <= 0.5 * minimum_support_reads_list[2]:
                #    correct_flag = 0
                if correct_flag == 0 :
                    candidate_single_SV_gt_fam_ls[0][i][qual_index] = "0"
                    candidate_single_SV_gt_fam_ls[0][i][gt_index] = "0/0"
                    candidate_single_SV_gt_fam_ls[0][i][gl_index] = "0,100,100,0,0,0,0,0,-4"
                elif correct_flag == 2 :
                    candidate_single_SV_gt_fam_ls[0][i][qual_index] = "255"
                    candidate_single_SV_gt_fam_ls[0][i][gt_index] = "1/1"
                    candidate_single_SV_gt_fam_ls[0][i][gl_index] = "100,100,0,1,1,0,0,0,-4"
                ...
                if gl_split_ls[8] in ["-1","-2","-3"] :
                    continue
                continue
            elif "0/0" == family_genotype_ls[0] and ("1/1" == family_genotype_ls[1] or "1/1" == family_genotype_ls[2]) and not ("1/1" == family_genotype_ls[1] and "1/1" == family_genotype_ls[2]):
                #if gl_split_ls[8] in ["-1","-2","-3"] :
                #    continue
                #if int(float(gl_split_ls[7])) >= 1 :
                ...
                #    candidate_single_SV_gt_fam_ls[0][i][gl_index] = ",".join(gl_split_ls)
                continue
            elif "1/1" == family_genotype_ls[0] and "0/0" == family_genotype_ls[1] and "0/0" == family_genotype_ls[2]:
                #continue
                correct_flag = False
                if gl_split_ls[8] == "-2" and abs(int(candidate_single_SV_gt_fam_ls[0][i][3])) > 5000 :
                    correct_flag = True
                elif gl_split_ls[8] in ["-1","-2","-3"] :
                    continue
                if abs(int(candidate_single_SV_gt_fam_ls[0][i][3])) > 1500 or candidate_single_SV_gt_fam_ls[0][i][0].lower() in ["x","chrx"] :
                    correct_flag = True
                gl_split_ls = candidate_single_SV_gt_fam_ls[0][i][gl_index].split(",")
                if int(float(gl_split_ls[6])) > 1 or int(float(gl_split_ls[7])) < 1.5 * minimum_support_reads_list[0] :
                    correct_flag = True
                if int(float(gl_split_ls[6])) / ( int(float(gl_split_ls[6])) + int(float(gl_split_ls[7])) ) > 0.75 :
                    correct_flag = True
                if correct_flag :
                    candidate_single_SV_gt_fam_ls[0][i][gt_index] = "0/1"
                    gl_split_ls = candidate_single_SV_gt_fam_ls[0][i][gl_index].split(",")
                    ...
                    candidate_single_SV_gt_fam_ls[0][i][gl_index] = ",".join(gl_split_ls)
                    continue
                #if candidate_single_SV_gt_fam_ls[0][i][0] == "1" and candidate_single_SV_gt_fam_ls[0][i][2] == "10407" :
                ...
                #    logging.info(gl_split_ls)
                #    logging.info(fa_split_ls)
                #    logging.info(mo_split_ls)
                
                
                
                a = int(float(fa_split_ls[6]))
                b = int(float(fa_split_ls[7]))
                if b >= min(2,minimum_support_reads_list[1]) or (a + b != 0 and b / (a + b) > 0.3 ):
                    candidate_single_SV_gt_fam_ls[1][i][gt_index] = "0/1"
                    fa_split_ls = candidate_single_SV_gt_fam_ls[1][i][gl_index].split(",")
                    fa_split_ls[0] = "100"
                    fa_split_ls[1] = "0"
                    fa_split_ls[2] = "100"
                    ...
                    fa_split_ls[4] = "0"
                    fa_split_ls[-1] = "-4"
                    candidate_single_SV_gt_fam_ls[1][i][gl_index] = ",".join(fa_split_ls)
                a = int(float(mo_split_ls[6]))
                b = int(float(mo_split_ls[7]))
                if b >= min(2,minimum_support_reads_list[2]) or (a + b != 0 and b / (a + b) > 0.3 ):
                    candidate_single_SV_gt_fam_ls[2][i][gt_index] = "0/1"
                    mo_split_ls = candidate_single_SV_gt_fam_ls[2][i][gl_index].split(",")
                    mo_split_ls[0] = "100"
                    mo_split_ls[1] = "0"
                    mo_split_ls[2] = "100"
                    ...
                    mo_split_ls[4] = "0"
                    mo_split_ls[-1] = "-4"
                    candidate_single_SV_gt_fam_ls[2][i][gl_index] = ",".join(mo_split_ls)
            elif "0/0" == family_genotype_ls[0] and "1/1" == family_genotype_ls[1] and "1/1" == family_genotype_ls[2]:
                if gl_split_ls[8] in ["-1","-2","-3"] :
                    continue
                continue
            else :
    ...
    for i in range(len(candidate_single_SV_gt_fam_ls[0])) :
        ...
        if resolution_denovo(family_gt_ls, family_gl_ls[0], family_mode) :
            ...
            if family_genotype_ls[0] in ["0/1","1/0"] and "0/0" == family_genotype_ls[1] and "0/0" == family_genotype_ls[2] :
                ...
                if 0 <= int(float(gl_split_ls[7])) <= 3 and 0 <= int(float(fa_split_ls[6])) <= 2 and 0 <= int(float(mo_split_ls[6])) <= 2 :
                    gl_split_ls[8] = "6"
            else :
                #gl_split_ls[8] = "5"
                pass
